...
df['ctxa'] = 'answer : ' + df['answer'] + ' context : ' + df['context']
...
class YourDataSetClass(Dataset):
  ...
  def __init__(self, dataframe, tokenizer, source_len, target_len, source_text, target_text):
    ...
    self.tokenizer.add_special_tokens(
        {'additional_special_tokens': ['<answer>', '<context>', '<question>']}
    )
    self.data = dataframe
...
global eval
eval  = [eval_bleu, eval_rouge, eval_meteor]
def evaluate(predictions:list, references:list):
    results = [eval[metric].compute(predictions=predictions, references=references)[metric] for metric in eval]
    return {'bleu': results[0], 'rouge': results[1], 'meteor': results[2]}

# %%
def T5Trainer(dataframe, source_text, target_text, model_params, output_dir="./t5-original/" ):
  ...
  for epoch in range(model_params["VAL_EPOCHS"]):
    ...
    try:
      res = evaluate(predictions, actuals)
      console.log(res)
    except:
      pass
...
# %%
T5Trainer(dataframe=df, source_text="ctxa", target_text="question", model_params=model_params, output_dir="outputs")
